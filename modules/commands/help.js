...
module.exports. run = function({ api, event, args, getText }) {
    ...
    const randomText = [ "Bot giúp mọi người tương tác, vui lòng không phá hoại tài sản của chung",];
    const text = `🔰Trang (${page}/${Math.ceil(arrayInfo.length/numberOfOnePage)})🔰\n🔰Gõ: "${prefix}help <tên lệnh>" để biết thêm chi tiết về lệnh đó🔰\n❗Hiện tại có ${arrayInfo.length} lệnh trên bot của Văn Mẫn link Facebook: https://www.facebook.com/profile.php?id=nvanman1605❗\n❗Dùng ${prefix}hép <Số trang>❗\n_________________________________\n[🤔🧐🤔🧐] : ${randomText[Math.floor(Math.random()*randomText.length)]}`;
